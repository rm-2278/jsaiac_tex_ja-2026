...
以下の8種類の報酬設計を比較した：
\begin{itemize}
  \item \textbf{flat（デフォルト）}: スコア増加ごとに一律+1.0の報酬
  \item \textbf{progressive}: スコア増加ごとに指数関数的な報酬（$2^{pos}$）
  \item \textbf{sequence\_bonus}: ベース報酬＋スコア増加ごとにボーナス
  \item \textbf{decaying}: 時間経過とともに減衰する報酬
  \item \textbf{sparse}: 完全なシーケンス完了時のみ報酬
  \item \textbf{progressive\_steep}: 急勾配な指数関数的報酬（$3^{pos}$）
  \item \textbf{dense\_guidance}: ステップごとの細かい報酬（正しい方向+0.1、間違ったタイル-0.1）
  \item \textbf{progress\_any}: flatと同様、スコア増加ごとに一律+1.0
\end{itemize}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\columnwidth]{media/pinpad/reward-design-sweep/sweep-subgoal-temporal.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\columnwidth]{media/pinpad/reward-design-sweep/sweep-heatmap-temporal.png}
