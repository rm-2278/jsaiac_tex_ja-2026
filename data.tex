...
\begin{figure}[t]
  ...
  \begin{subfigure}[b]{\columnwidth}
    ...
    \caption{探索位置のヒートマップ (Heatmap)}
  \end{subfigure}
  
  \caption{報酬割り当て係数の変更による学習への影響。}
  \label{fig:reward-sweep-analysis}
\end{figure}

% --- 縦長の図は独立させて配置（高さ制限をつける）---
\begin{figure}[t]
  \centering
  % keepaspectratioで歪みを防ぎつつ、高さをページ高の80%に制限
  \includegraphics[height=0.85\textheight, keepaspectratio]{media/pinpad/reward-ratio-sweep/sweep-subgoal-temporal.png}
  \caption{サブゴールの時間的変化。特定の報酬設定においてサブゴールが固定化される傾向が見られる。}
  \label{fig:reward-sweep-subgoal}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\columnwidth]{media/pinpad/entropy-sweep/sweep-subgoal-temporal.png}
  \caption{エントロピー調整実験におけるサブゴール可視化の時間的変化。}
...
\begin{itemize}
  ...
  \item \textbf{progressive}: タイルを踏んだときの末尾の一致度の増減を指数関数的にしたものが報酬（$2^{tail-match}$）
  ...
  \item \textbf{progressive\_steep}: 急勾配な指数関数的報酬（$3^{tail-match}$）
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\columnwidth]{media/pinpad/reward-design-sweep/sweep-subgoal-temporal.png}
  \caption{報酬設計実験におけるサブゴール可視化の時間的変化。}
  \label{fig:reward-design-subgoal-temporal}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\columnwidth]{media/pinpad/reward-design-sweep/sweep-heatmap-temporal.png}
...
\begin{figure*}[t]
  ...
  \caption{Atari 100kベンチマークにおける学習結果。上段が学習曲線、下段が方策の可視化を示す。Freewayなどでは早期に方策が固定化されている様子が、大きな画像で確認できる。}
  \label{fig:atari-analysis}
\end{figure*}

図\ref{fig:atari-analysis}に示すように、HierosのAtari環境における学習は、スコアの観点では論文通り高い性能を示しているが、方策の可視化を行うと単純な行動パターンに収束していることが確認される。
特にFreeway(a)(b)では、スコアは高いものの方策画像を確認すると「前進（Up）」動作のみが学習されており、複雑な方策は学習できていない。
