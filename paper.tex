...
本研究では，階層的世界モデルの代表例であるHieros\cite{2}に着目し，性能評価と内部状態の可視化を通じてその実態を検証した。
Visual Pinpad環境での実験により，Hierosはハイパーパラメータ設定に対して高い感度を示し，報酬設計や更新頻度の変更に対する頑健性に限界があることがわかった。
また，Atari環境での方策可視化では，高スコアを示しているにも関わらず単純な行動パターンのみが学習されており，階層性を活かしたサブゴールの学習が実現されていないことを確認した。
さらに，\texttt{max\_hierarchy}パラメータの比較実験により，階層数の増加が学習の安定性を低下させることが確認された。
これらの結果は，現在の階層的世界モデルには理論的期待と実際の性能の間にギャップがあることを示し，より頑健な階層的学習手法の必要性を示唆している。
\end{abstract}
...
本研究の実験結果により，Hierosの性能がタスク特性，報酬設計，更新頻度，報酬関数係数，階層数などのハイパーパラメータに強く依存することが明らかになった。特にAtari環境では，スコア上は高い性能を示しているものの，実際に学習された方策は単調な行動パターンに留まっていることを確認した。
...
図~\ref{fig:hieros-baseline}に示すように，400,000ステップの学習期間を通じて累積報酬が0付近に留まり，有意な学習進展が観察されなかった。
...
主な研究成果は以下の通りである：
\begin{enumerate}
\item Visual Pinpad環境での実験では，Hierosがハイパーパラメータの設定に強く依存し，サブゴール更新頻度、報酬係数、エントロピーの調整に対して限定的な頑健性しか示さないことが明らかになった。

\item Atari環境での方策可視化実験では，高いスコアを達成しているにも関わらず、実際には単純な行動パターンの繰り返しのみが学習されており，階層性が活用されていないことを確認した。

\item \texttt{max\_hierarchy}パラメータの比較実験により，階層数を増やすことで学習の安定性が低下し，理論上の期待と実際の性能にギャップがあることが明らかになった。

\item DirectorとHierosの比較実験から，世界モデルの階層化による複雑性の増大が学習の困難さを高めている可能性が示唆された。
\end{enumerate}

これらの結果は，現在の階層的世界モデルには根本的な課題があることを示しており，今後の研究方向として以下を提案する：(1) ハイパーパラメータに対する頑健性の向上，(2) 動的な階層的抽象化の実現，(3) 世界モデル階層化に伴う最適化問題の難しさの理論的説明。本研究の成果は，階層的強化学習と世界モデルの結合における今後のアルゴリズム設計のための基礎的な知見を提供するものである。
