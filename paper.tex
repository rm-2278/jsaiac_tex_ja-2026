...
\etitle{Empirical Re-evalutaion of the Hierarchical World Model Hieros: Internal Representation Analysis and the Impact of Hierarchical Structure}}
...
深層強化学習は多くの分野で成功を収めている一方で，大量の環境相互作用を必要とするサンプル効率の低さや，長期的依存を伴うタスクにおける学習の不安定性が依然として課題である \cite{12,8}。
この課題に対し，環境ダイナミクスを潜在空間上に学習し，将来状態を予測するワールドモデルが提案されてきた \cite{13,14}。
特に Dreamer 系列 \cite{3,4,5} は，潜在状態空間上での想像学習により高いサンプル効率を実現している。
さらに，長期的な依存関係を扱うために，時間抽象化を導入する階層的強化学習 (Hierarchical Reinforcement Learning, HRL) が研究されてきた~\cite{12,8}。
...
HRL は，エージェントが不確実な情報を集める行動をする探索において有益であると知られており~\cite{8}，効率的な報酬割り当て，転移学習，高い解釈性などといった点でも優位性があると考えられている~\cite{12}。
...
この２つの要素を組み合わせた数少ないモデルとして，Director \cite{1} や Hieros \cite{2} があり，ベースラインとして使用されている Dreamer などのフラットな世界モデルを上回る性能を示している。
...
\end{enumerate}
Hierosにおいて、各階層毎に以下のように定義される3つの報酬関数を持つ：
\begin{align*}
...
世界モデルとは，環境の遷移ダイナミクスを学習し，潜在空間上で将来状態を予測することで方策学習を効率化する枠組みである~\cite{13,14}。
強化学習において環境はマルコフ決定過程（MDP）\cite{17} として定式化される：
\begin{equation}
...
世界モデルは遷移確率 $P$ を近似することで潜在空間上で軌道生成（imagination）を可能にする~\cite{13,3}。
...
状態予測モデルと観測予測モデルは直接的な予測を行うが，学習の安定性や表現効率の面で課題があるとされており~\cite{13}，近年は特に状態空間モデルが注目されている~\cite{14}。
...
次に，モデルの変化を見やすくするため，Pinpad の報酬設計を元の実装から，末尾の一致度が上がるとスコアが与えられるように変更した Pinpad-easy というタスクを作成した。以降の実験ではこのタスクを用いることとする。
例えば，赤緑青が正解シーケンスである Pinpad-easy-three タスクでは，赤 $\rightarrow$ 赤緑に変化した場合は +1 の報酬，赤緑 $\rightarrow$ 赤緑緑に変化した場合は -2 の報酬が与えられる）。
また，Hieros 　の内部状態について調査するために，Hieros の各レイヤが提示しているサブゴールの可視化を行った。元の Hieroes の実装~\cite{9} では過去の経験を参考にしたノイズを加えてサブゴールを可視化していたが，上位モデルの提示する情報のみに焦点を当てるために，ノイズなしで可視化した。
...
\begin{figure}[!h]
  ...
  \caption{Pinpad-easy\_threeにおける報酬設計ごとの学習曲線。}
...
Director を用いた Pinpad および Pinpad の結果を示す。
...
ここから，VPRやTHICK　\cite{15,16}のように更新頻度を可変長にすること，または HarmonyDream~\cite{11} のように報酬を自動でバランスすることで，より多様なタスクを学習できるようになると予想できる。
\\
...
\bibitem{9} \url{https://github.com/Snagnar/Hieros}
\bibitem{10} \url{https://github.com/danijar/director}
\bibitem{11} Ma, H. et al.: HarmonyDream: Task Harmonization Inside World Models, ICML (2024).
\bibitem{12} Pateria, S. et al.: Hierarchical Reinforcement Learning: A Comprehensive Survey, ACM Computing Surveys (2021).
\bibitem{13} Ha, D. and Schmidhuber, J.: World Models, NeurIPS (2018).
\bibitem{14} Hafner, D. et al.: Learning Latent Dynamics for Planning from Pixels, ICML (2019).
\bibitem{15} Gu, A. et al.: Efficiently Modeling Long Sequences with Structured State Spaces, ICLR (2022).
\bibitem{16} Smith, J. T. et al.: Simplified State Space Layers for Sequence Modeling, ICLR (2023).
\bibitem{17} Sutton, R. S. and Barto, A. G.: Reinforcement Learning: An Introduction, MIT Press (2018).
