...
%%
\begin{abstract}
\textcolor{red}{（アブスト相当の部分では引用文献はなくても良い気がします。max_hierarchy パラメータも突然なので，階層数（の最大値）で良いと思います。）}
...
また，Atari環境での方策可視化では，高スコアを示しているにも関わらず単純な行動パターンのみが学習されており，階層性を活かしたサブゴールの学習が実現されていないことを確認した。
さらに，
% \texttt{max\_hierarchy}パラメータ
階層数の比較実験により，階層数の増加が学習の安定性を低下させることが確認された。
...
深層強化学習は多くの分野で成功を収めている一方で，大量の環境相互作用を必要とするサンプル効率の低さや，長期的依存を伴うタスクにおける学習の不安定性が課題である \cite{14,9}。
これらの課題に対し，環境ダイナミクスを潜在空間上に学習し，内部モデル\textcolor{red}{（「内部モデル」と「潜在空間」の明確な使い分けがなければ潜在空間に統一が良い気がします）}上で将来状態を予測する世界モデルが提案されてきた \cite{15,16}。
特に Dreamer 系列~\cite{3,4,5}は，潜在状態空間上での想像学習により高いサンプル効率を実現している。
さらに，長期的な依存関係を扱うために，時間抽象化を導入する階層的強化学習 (Hierarchical Reinforcement Learning, HRL) が研究されてきた~\cite{14,9}。
近年では，世界モデルと階層構造を統合した手法が提案されており，多時間スケールでの計画と想像を同時に実現する枠組みである Director~\cite{1} や Hieros~\cite{2} は，\textcolor{red}{Dreamer など？の既存の}の世界モデルを上回る性能を示している点で注目されている。
...
HRL は，エージェントが不確実な情報を集める行動をする探索において有益であると知られており~\cite{9}，効率的な報酬割り当て，転移学習，高い解釈性などといった点で\textcolor{red}{も？}優位性があると考えられている~\cite{14}。
また，世界モデルは環境もモデル化することで高コストなエージェントと環境の相互作用を減らし，高いサンプル効率を実現する。
これらを組み合わせた階層的世界モデルの研究も近年進められており，例えば Director~\cite{1} や Hieros~\cite{2} は特定の分野で既存の世界モデルを上回る性能を示している。

Director は Dreamer という潜在空間での創造によって効率的な学習を行う世界モデル~\cite{3,4,5} を基盤として，方策を決めるモデルに Managerと Worker の二層構造を導入した手法である。Manager は長期的な潜在目標を生成し，Worker は短期的行動を出力するモデルであり，Worker の損失関数に目標達成度を組み込むことで，同時に学習を行う。

Hieros は次の 4 つの点を改良するように Director を拡張した手法である：
\begin{enumerate}
   \item 方策を決めるモデルだけでなく世界モデルも階層化し，階層ごとに異なる時間スケールを扱えるようにする。
   \item 階層数を三層以上への拡張する。
   \item 潜在空間のダイナミクスの学習で再帰型状態空間モデル (Recurrent State-Space Model, RSSM) の代わりに\textcolor{red}{（線形状態空間モデルに基づく状態空間モデル？ (Simplified Structured State Space Sequence World Model, S5WM)正しいか未チェックです）} を採用して\textcolor{red}{より長期的な依存関係を学習できるようにする。??}
   \item 時間計算量を削減するサンプリング戦略を導入する。
\end{enumerate}
Atari ベンチマークという〜において DreamerV3 を上回る性能が報告されている一方で，学習された表現の解釈性やモデルサイズをスケールする方向の研究は不十分であり，階層的な世界モデルの理論および評価実験は知られていない。
...
強化学習において環境はマルコフ決定過程~\cite{20} として定式化される：
\begin{equation}
\mathcal{M} = (\mathcal{S}, \mathcal{A}, P, r, \gamma)
\end{equation}
ここで，$\mathcal{S}$ は状態空間，$\mathcal{A}$ は行動空間である。時刻 $t$ における状態 $s_t$ と行動 $a_t$ から次時刻の状態 $s_{t+1}$ への遷移確率を $P(s_{t+1}\mid s_t, a_t)$ と表す。報酬関数 $r(s_t, a_t)$ は状態行動ペア $(s_t, a_t)$ に対して報酬を返す。$\gamma \in [0,1)$ は将来報酬に対する割引率を表す。
...
世界モデルは潜在状態において次状態を予測することを目的とする状態予測モデル，高次元な観測空間において次の観測を予測することを目的とする観測予測モデル，そして観測を低次元の潜在空間に写像しその潜在空間上でダイナミクスを学習する状態空間モデルの3種類に大別される~\cite{15}。
状態予測モデルと観測予測モデルは直接的な予測を行うが，学習の安定性や表現効率の面で課題があるとされており~\cite{15}，近年は特に状態空間モデルが注目されている~\cite{16}。
...
Dreamer が imagination \textcolor{red}{（ここの imagination とは生成された軌道を指しますか？少し日本語を細くしたいですが，理解が足りず難しそうです…）} に基づく方策学習を主軸とするのに対し，TD-MPC2 は学習された潜在モデルを用いてオンラインで行動系列を計画する点に特徴がある。
...
Visual Pinpad は，エージェントがタイルを踏むことで色が変化する環境であり，特定の色のシーケンスを達成することが目的のタスクである\textcolor{red}{（正しいでしょうか？）}。
...
次に，
% 単純な Pinpad 設計では，報酬がフルシーケンスに到達するまで発生せず難しいため，
モデルの変化を見やすくするため，Pinpad の報酬設計を元の実装から，末尾の一致度が上がるとスコアが与えられるように変更した Pinpad-easy というタスクを作成した。以降の実験ではこのタスクを用いることとする。
例えば，赤 $\rightarrow$ 赤緑に変化した場合は +1 の報酬，赤緑 $\rightarrow$ 赤緑緑に変化した場合は -2 の報酬が与えられる\textcolor{red}{（ここはあまりわかっていないのですが，何が正解であり，前者はなぜプラスの報酬が得られ，後者はなぜマイナスなのかがわかるような簡潔な文が良い気がします）}。
また，Hieros の内部状態について調査するために，Hieros の各レイヤが提示しているサブゴールの可視化を行った。
元の Hieroes の実装~\cite{11} では過去の経験を参考にしたノイズを加えてサブゴールを可視化していたが，上位モデルの提示する情報のみに焦点を当てるために，ノイズなしで可視化した。
また，Hieros エージェントの各ステップにおける，各地点への累積訪問割合もヒートマップとして可視化した。
...
\begin{figure}[!htbp]
  ...
  \caption{Pinpad-easy：探索位置ヒートマップの時間的変化。各設定における累積訪問位置の分布。}
...
例えば，\texttt{subactor\_update\_every=4} は，4 ステップごとに上位階層が下位階層に与えるサブゴールを更新していることを意味する。
...
また，図~\ref{fig:subgoal-temporal} において上位層が提示するサブゴールも，上位2階層ともに赤までしか提示しておらず，求めている赤緑青のフルシーケンスを提示するようには学習されなかった。
同様に，図~\ref{fig:heatmap-temporal} においても探索範囲が収束している様子が確認できた。
...
次に方策エントロピーを変更する実験の結果を図~\ref{fig:entropy-heatmap-temporal}, \ref{fig:entropy-sweep-scores} に示す。
図の判例において，方策エントロピーが単一の値の場合すべての階層で同一の値であることを表し，3つ値がある場合は左から順に下位，中位，上位階層のエントロピー値を示す。
...
\begin{itemize}
  ...
  \item \textbf{progressive}: タイルを踏んだときの末尾の一致度の増減を指数関数的にしたもの（$2^{\text{tail-match}}$）を報酬として与える。
  \item \textbf{sequence\_bonus}: flat の報酬に加え，末尾一致度が増加ごとにボーナスで報酬を与える。
  \item \textbf{decaying}: 時間経過とともに報酬を減衰させる。
  \item \textbf{sparse}: 元の Pinpad 環境よ同様に，完全なシーケンス一致時のみ報酬を与える。
  \item \textbf{progressive\_steep}: 急勾配な指数関数的報酬（$3^{\text{tail-match}}$）を与える。
...
Director を用いた Pinpad-three および Pinpad-easy-three の結果を示す \textcolor{red}{（three は  3 で良いでしょうか？また，なにが 3 つになったのか簡潔に述べられるでしょうか）}}。
Director ではどちらの報酬設計でも学習が進んでいることが確認される。Pinpad-3 の結果は~\cite{1} で報告されている結果に近いものとなっている。
また，比較のために作成した Pinpad-dense での学習も行ったところ，高い性能が出ることを確認でき，報酬設計には問題がないことが確認された。
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.7\textwidth]{media/hierarchy/hierarchy_episode_scores.png}
...
ここから，更新頻度を可変長にすること，または HarmonyDream~\cite{13} のように報酬を自動でバランスすることで，より多様なタスクを学習できるようになると予想できる。
\\
...
本研究では，階層的世界モデルHierosの性能を詳細に評価し，内部状態の可視化を通じてその実態を検証した。

主な研究成果は以下の通りである：
\begin{enumerate}
\item Visual Pinpad 環境での実験によって，Hieros がハイパーパラメータに敏感であり，サブゴール更新頻度，報酬係数，エントロピーの調整に対して限定的な頑健性しか示さないことが明らかになった。

\item Atari 環境での方策可視化実験では，高いスコアを達成しているにも関わらず，実際には単純な行動パターンの繰り返しのみが学習されており，階層性が活用されていないことが確認された。

\item 階層数の上限を決める \texttt{max\_hierarchy} パラメータの比較実験により，階層数を増やすことで学習の安定性が低下し，理論上の期待と実際の性能にギャップがあることが明らかになった。

\item DirectorとHierosの比較実験から，世界モデルの階層化による複雑性の増大が学習の困難さを高めている可能性が示唆された。
\end{enumerate}

これらの結果は，現在の階層的世界モデルには根本的な課題があることを示唆している。
よって，今後の研究方向として以下を提案する：(1) ハイパーパラメータに対する頑健性の向上，(2) 動的な階層的抽象化の実現，(3) 世界モデル階層化に伴う最適化問題の難しさの理論的説明。
本研究の成果は，階層的強化学習と世界モデルの結合における今後のアルゴリズム設計のための基礎的な知見を提供することを期待する。
...
表~\ref{tab:pinpad-hparams}にあるものをベースとし，各実験において1つまたは２つの値を変更しながら組み合わせを試した。
表に記述されていないものは，\cite{2} と同じ値を使用した。
\begin{table}[t]
  \centering
  \begin{tabular}{ll}
    \hline
